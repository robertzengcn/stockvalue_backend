...
class YieldGapDataBase(BaseModel):
    ...
    gross_dividend_yield: float = Field(
        ..., ge=0, description="Gross dividend yield (before tax)"
    )
    net_dividend_yield: float = Field(
        ..., ge=0, description="Net dividend yield (after tax)"
    )
    risk_free_bond_rate: float = Field(
        ..., ge=0, description="10-year treasury bond rate"
    )
    risk_free_deposit_rate: float = Field(
        ..., ge=0, description="3-year large deposit rate"
    )
    yield_gap: float = Field(
        ..., description="Yield gap: net_yield - max(bond, deposit)"
    )
    recommendation: YieldRecommendation = Field(
